...
- **A bump in a repo that commits no lockfile** (libraries, most published packages) → the subtree still moves, it is just invisible: `depsound transitive npm:<name> <from> <to>` projects it (npm/crates, via deps.dev). Resolved in isolation, so it is an upper bound on what the bump adds to *this* project, and a live snapshot, not reproducible like a diff; for the exact set, generate a lockfile pair (`depsound guide`, no package code runs).
...
- **Provenance** (npm/crates, runs by default). Deltas vs the package's own history. A WARNING, an install script newly ADDED, attestation DROPPED, repo MISMATCH, or a YANKED version, is the account-takeover shape and a strong reason to hold. Read a publisher change by its DIRECTION: onto a trusted publisher (OIDC, usually with an attestation appearing) the maintainer retired a long-lived token, which is hardening, not a flag; off one, back to a user account, is the takeover tell. Freshness/size/dormancy notes are context, not alarms. A clean panel is not a pass; the checks are shallow.
...
- **Unreviewable mass is relative to what you consume.** Packages often ship a browser bundle beside consumable ESM/CJS modules; check depsound's resolved entrypoints. Entrypoints outside the mass and a side-bundle: context, not payload. Entrypoint inside the mass (or an action's dist/): that mass IS the runtime, full weight. Say which case applies.
- **Never hand back a raw signal.** Before deferring on a residual (a publisher change, a fresh major), spend the cheap investigation: releases/blog for an announced pipeline change, the new publisher's profile, attestation-vs-repo crosscheck. Narrative is attacker-writable; it only downgrades a signal when hard facts corroborate. Then give a recommendation with the residual named; the decision stays the owner's.
- **The transitive subtree.** A single-pair diff covers the top dep only. `depsound transitive` shows the whole subtree the bump moves, from a lockfile pair or projected when there is none; review the added/changed modules the same way. Read the causality, not just the count: each added or removed module names the direct dep that pulls it in, so "20 new packages" resolves to *which* dependency dragged them in and whether you accept that dependency growing that way.
